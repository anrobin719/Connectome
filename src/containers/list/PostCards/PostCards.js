...
class PostCards extends Component {
  componentDidMount() {
    const {
      ...
      tag2,
      posts,
      onSetPostListViaTitle,
      onSetPostListViaTags,
      onGetPostList
    } = this.props;
    if (search || tag1) {
      if (search) {
  ...
  render() {
    ...
    return (
      <Fragment>
        {(search || tag1 || tag2) && filteredPosts.size === 0 ? (
          <div className={classes.searchArr}>검색 결과가 없습니다.</div>
        ) : (
          <div className={classes.PostCards}>{postCards}</div>
        )}
      </Fragment>
    );
  }
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    onSetPostListViaTitle: (searchVal, posts) =>
      dispatch(actions.setPostListViaTitle(searchVal, posts)),
    onSetPostListViaTags: (tag1, tag2, posts) =>
      dispatch(actions.setPostListViaTags(tag1, tag2, posts))
  };
};
